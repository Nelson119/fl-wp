...
class Wpcc_Admin {
	...
	function __construct() {
		...
		if( !empty($_GET['lang']) && in_array($_GET['lang'], array_keys($this->langs)) )
			$this->admin_lang = $_GET['lang'];
...
<?php foreach($this->langs as $key => $value) { ?>
	<input type="checkbox" id="wpcco_lang_<?php echo $key; ?>" name="wpcco_lang_<?php echo $key; ?>"<?php echo in_array($key, $this->options['wpcc_used_langs']) ? ' checked="checked"' : ''; ?> />
	<label for="wpcco_lang_<?php echo $key; ?>"><?php $str = $value[2] . ' (' . $key . ')'; echo str_replace(' ', '&nbsp;', str_pad($str, 14 + strlen($str) - mb_strlen($str)));?></label>
...
<td>
<select id="wpcco_use_cookie_lang" value="" name="wpcco_use_cookie_lang" style="width: 250px;">';
<option value="2"<?php echo $this->options['wpcc_use_cookie_lang'] == 2 ? ' selected="selected"' : ''; ?>>直接显示对应繁简版本内容</option>
<option value="1"<?php echo $this->options['wpcc_use_cookie_lang'] == 1 ? ' selected="selected"' : ''; ?>>跳转到对应繁简版本页面</option>
<option value="0"<?php echo $this->options['wpcc_use_cookie_lang'] == 0 ? ' selected="selected"' : ''; ?>>关闭此功能(默认值)</option>
</select>
...
<label><input type="radio" name="wpcco_use_permalink" value="0"<?php echo $this->options['wpcc_use_permalink'] == 0 ? ' checked="checked"' : ''; ?> /> <code><?php echo get_option('home') . (empty($wp_rewrite->permalink_structure) ? '/?p=123&lang=zh-tw' : $wp_rewrite->permalink_structure . '?lang=zh-tw'); ?></code> (默认)</label><br />
...
默认的形式为您原始Permalink后加上?lang=zh-tw参数.(zh-tw为当前请求的语言代码) .你可以修改这个permalink形式.本插件提供两种非默认的Permalink格式:
...
<tr><td valign="top" width="30%"><?php
	...
	else if( empty($this->options['wpcc_browser_redirect']) && empty($this->options['wpcc_use_cookie_lang']) )
...
<input class="button" type="submit" name="submit" value="<?php
	if( $this->get_cache_status() == 0 )
		...
		echo "卸载兼容";
 ?>" <?php
	if( $this->get_cache_status() == 0 || ( empty($this->options['wpcc_browser_redirect']) && empty($this->options['wpcc_use_cookie_lang']) ) )
...
<?php
	...
	function navi() {
		...
		$str = '<span><a title="默认/ 默認" href="' . $this->url . '" ' . ( !$lang ? 'style="color: #464646; text-decoration: none !important;"' : '' ) . ' >默认/ 默認</a></span>&nbsp;';
		if(!$this->options['wpcc_used_langs']) return $str;
			foreach($this->langs as $key => $value) {
			$str .= '<span><a href="' . $this->url . '&lang=' . $key . '" title="' . $value[2] . '" ' . ( $lang == $key ? 'style="color: #464646; text-decoration: none !important;"' : '' ) . '>' . $value[2] . '</a>&nbsp;</span>';
		}
		return $str;
	}
	...
	function process() {
		...
		$langs = array();
		foreach($this->langs as $key => $value) {
			if(isset($_POST[ 'wpcco_lang_' . $key ]))
				$langs[]=$key;
		}
		$options = array(
			...
			'wpcc_use_cookie_lang' => intval($_POST['wpcco_use_cookie_lang']),
		...
		if( $this->get_cache_status() == 2 && empty($options['wpcc_browser_redirect']) && empty($options['wpcc_use_cookie_lang']) ) {
