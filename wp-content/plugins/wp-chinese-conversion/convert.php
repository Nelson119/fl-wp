 ...
 * 
 * 通過include本文件, 你可以使用中文繁簡轉換函數zhconversion($str, $lang)
 * 如果$_GET['doconversion']或$_POST['doconversion'])有設置, 本文件將獲取$_REQUEST['data']并把其轉換為$_REQUEST['lang']語言后輸出.
 *
...
$wpcc_langs = array(
	...
	'zh-cn' => array('zhconversion_cn', 'zh2CN', '中国简体'),
...
{	$wpcc_data = get_magic_quotes_gpc() ? stripslashes($_REQUEST['data']) : $_REQUEST['data'];
	$wpcc_lang = str_replace('_', '-', strtolower(trim($_REQUEST['lang'])));
	if( !empty($wpcc_lang) && in_array($wpcc_lang, array('zh-hans', 'zh-hant', 'zh-cn', 'zh-hk', 'zh-sg', 'zh-tw', 'zh-my', 'zh-mo')) )
		echo zhconversion($wpcc_data, $wpcc_lang);
	else echo $wpcc_data;
	die();
}
}

function zhconversion($str, $lang) {
	global $wpcc_langs;
