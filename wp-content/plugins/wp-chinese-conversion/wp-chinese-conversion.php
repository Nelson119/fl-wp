...
$wpcc_langs = array(
	...
	'zh-cn' => array('zhconversion_cn', 'cntip', '中国简体','zh-CN'),
...
if( $wpcc_options != false && is_array($wpcc_options) && is_array($wpcc_options['wpcc_used_langs']) ) {
	...
	add_filter('query_vars', 'wpcc_insert_query_vars');//修改query_vars钩子,增加一个'lang'公共变量.
...
function wpcc_init() {
	...
	if( (strpos($_SERVER['PHP_SELF'], 'wp-comments-post.php') !== false
		...
		!empty($_POST['lang']) && in_array($_POST['lang'], $wpcc_options['wpcc_used_langs'])
	) {
		global $wpcc_target_lang;
		$wpcc_target_lang = $_POST['lang'];
		wpcc_do_conversion();
		return;
	}
...
function wpcc_parse_query_fix($this_WP_Query) {
		...
		if ( $this_WP_Query->is_home && 'page' == get_option('show_on_front') && get_option('page_on_front') ) {
			...
			if ( empty($_query) || !array_diff( array_keys($_query), array('preview', 'page', 'paged', 'cpage', 'lang') ) ) {
 ...
 * 举例, 当访客在一个繁简转换页面提交搜索时, 本插件载入的JS脚本会在GET请求里附加一个lang变量,
 * 如 /?s=test&lang=zh-tw
...
//]]>
	...
	if( !$wpcc_direct_conversion_flag )
		wp_enqueue_script('wpcc-search-js', WPCC_ROOT_URL . 'search-lang.js', array(), '1.0', false);
		//echo '<script type="text/javascript" src="' . WPCC_ROOT_URL . 'search-lang.js' . '"></script>';
...
function wpcc_template_redirect() {
	...
	function wpcc_modify_comment_form() {
		global $wpcc_target_lang;
		echo '<input type="hidden" name="lang" value="' . $wpcc_target_lang . '" />';
	}
	wpcc_do_conversion();
}

/**
 * 在Wordpress的query vars里增加一个lang变量.
 *
 */
 ...
 * @param string $lang optional, Default to null, chinese language code you want string to be converted, if null( default), will use $GLOBALS['wpcc_target_lang']
 ...
 *
 */
function zhconversion($str, $lang = null) {
	global $wpcc_options, $wpcc_langs;
	if( $lang === null ) $lang = $GLOBALS['wpcc_target_lang'];
	if( $lang == false) return $str;

	//if( !empty($wpcc_options['wpcc_no_conversion_tag']) || $wpcc_options['wpcc_no_conversion_ja'] == 1 )
	//	return limit_zhconversion($str, $wpcc_langs[$lang][0]);
	return $wpcc_langs[$lang][0]($str);
}


function zhconversion2($str, $lang = null) { // do not convert content within <!--WPCC_NC_START--> and <!--WPCC_NC_END-->.
	global $wpcc_options, $wpcc_langs;
	if( $lang === null ) $lang = $GLOBALS['wpcc_target_lang'];
	if( $lang == false) return $str;
 ...
 *
 */
function zhconversion_safe($str, $lang = null) {
	wpcc_load_conversion_table();
	return zhconversion($str, $lang);
}
 ...
 * 否则, 增加一条 '.../zh-tw|zh-hant|...|zh-hans|zh|zh-reset/?$' => 'index.php?var1=$matches[1]..&varN=$matches[N]&lang=$matches[N+1]'的新规则
	...
	if($wpcc_options['wpcc_use_permalink'] == 1) {
		foreach($rules as $key => $value) {
			...
			if( substr($key, -3) == '/?$' ) {
				...
				$rules2[substr($key, 0, -3) . '/(' . $reg . '|zh|zh-reset)/?$'] = $value . '&lang=$matches[' . $number . ']';
			}
		}
	}
		...
		foreach($rules as $key => $value) {
			if( strpos($key, 'trackback') !== false || strpos($key, 'print') !== false || strpos($value, 'lang=') !== false ) continue;
			if( substr($key, -3) == '/?$' ) {
				$rules2['(' . $reg . '|zh|zh-reset)/' . $key] = preg_replace_callback('/\$matches\[(\d+)\]/', '_wpcc_permalink_preg_callback', $value) . '&lang=$matches[1]';
			}
		}
	}
	$rules2['^(' . $reg . '|zh|zh-reset)/?$'] = 'index.php?lang=$matches[1]';//首页的繁简转换版本rewrite规则
 ...
 *
 */
function wpcc_link_conversion($link, $lang = null) {
	global $wpcc_options;
	...
	if( $lang === null ) $lang = $GLOBALS['wpcc_target_lang'];
	if( $lang == false ) return $link;

	if( strpos($link, '?') !== false || !$wpcc_options['wpcc_use_permalink'] )
		return add_query_arg('lang', $lang, $link);
	if($wpcc_options['wpcc_use_permalink'] == 1)
		return user_trailingslashit(trailingslashit($link) . $lang);
	return preg_replace('#^(http(s?)://[^/]+' . $wpcc_wp_homepath . ')#', '\\1' . $lang . '/', $link);
}
 ...
 * @since 1.1.14.2
 */
function wpcc_link_conversion_auto($link, $lang = null) {
...
function wpcc_get_noconversion_url() {
	...
	$tmp = ( is_ssl() ? 'https://' : 'http://' ) .
		...
		$_SERVER['REQUEST_URI'];
	$tmp = trim(strtolower(remove_query_arg('lang', $tmp)));
...
function wpcc_fix_link_conversion($link) {
	...
	if( $wpcc_options['wpcc_use_permalink'] == 1 ) { 
		...
		if( preg_match('/^(.*)\?lang=([-a-zA-Z]+)\/(.*)$/', $link, $tmp) )
			return add_query_arg('lang', $tmp[2], trailingslashit($tmp[1]) . $tmp[3]);
		return $link;
	}
	else
		return $link;
 ...
 */
function lang_attribute($default = "zh", $lang = false) {
	global $wpcc_langs;
	if( !$lang )
		$lang = $GLOBALS['wpcc_target_lang'];
	if( !$lang )
		return $default;
	return $wpcc_langs[$lang][3];
}
/**
 * 返回當前語言代碼
 * @since 1.1.9
 */
function lang($default = false) {
	global $wpcc_target_lang;
	if( !$wpcc_target_lang )
		return $default;
...
function wpcc_output_navi($args = '') {
	...
	if( ( $wpcc_options['wpcc_browser_redirect'] ==2 || $wpcc_options['wpcc_use_cookie_lang'] == 2 ) &&
		$wpcc_target_lang
	) {
...
function wpcc_output_navi2() {
	...
	$output .= '	<span id="wpcc_cn_link" class="'. ( $wpcc_target_lang == 'zh-cn' ? 'wpcc_current_lang' : 'wpcc_lang' ) . '" ><a class="wpcc_link" rel="nofollow" href="'. esc_url($wpcc_langs_urls['zh-cn']) . '" title="中国简体" >中国简体</a></span>' ."\n";
...
function wpcc_filter_search_rule($where) {
	...
	if( empty($wp_query->query_vars['sentence']) && count($wp_query->query_vars['search_terms']) > 1 && $wp_query->query_vars['search_terms'][0] != $wp_query->query_vars['s'] ) {
		...
		if(!empty($sa)) {
			foreach($sa as $value) {
				$or .= "{$and3}( ". $wpdb->prefix ."posts.post_title LIKE '%" . esc_sql($value) . "%') ";
				$or .= " OR ( ". $wpdb->prefix ."posts.post_content LIKE '%" . esc_sql($value) . "%') ";
				$and3 = ' OR ';
			}
		}
	}
	*/
	...
	// debug
	// echo("Where: ". $where . "<br /><br />Search: " . $original . "<br /><br />Replace with: $or");die();
 ...
 *
 * @todo 彻底重写本函数（目前是一团浆糊）。使用Wordpress原生的query var系統讀/寫lang參數, 1.2版本實現. 
...
function wpcc_parse_query($query) {
	...
	$request_lang = isset($query->query_vars['lang']) ? $query->query_vars['lang'] : '';
	$cookie_lang = isset($_COOKIE['wpcc_lang_' . COOKIEHASH]) ? $_COOKIE['wpcc_lang_' . COOKIEHASH] : '';
	...
	if(!$wpcc_target_lang) {
		if( $request_lang == 'zh' ) {
			if( $wpcc_options['wpcc_use_cookie_lang'] != 0 ) {
				setcookie('wpcc_lang_' . COOKIEHASH, 'zh', time() + 30000000, COOKIEPATH, COOKIE_DOMAIN);
			} else {
				setcookie('wpcc_is_redirect_' . COOKIEHASH, '1', 0, COOKIEPATH, COOKIE_DOMAIN);
			}
		...
		if( $request_lang == 'zh-reset' ) {
			setcookie('wpcc_lang_' . COOKIEHASH, '', time() - 30000000, COOKIEPATH, COOKIE_DOMAIN);
		...
		if($wpcc_options['wpcc_use_cookie_lang'] != 0 && !$is_robot && $cookie_lang) {
			if(in_array($cookie_lang, $wpcc_options['wpcc_used_langs'])) {
				if( $wpcc_options['wpcc_use_cookie_lang'] == 2 ) {
	...
	if( $wpcc_target_lang && $wpcc_options['wpcc_use_cookie_lang'] != 0 && $cookie_lang != $wpcc_target_lang ) {
		setcookie('wpcc_lang_'. COOKIEHASH, $wpcc_target_lang, time() + 30000000, COOKIEPATH, COOKIE_DOMAIN);
	}
